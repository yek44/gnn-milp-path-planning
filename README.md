# GCNN Branching for MILP Path Planning

Graph convolutional branching policies for mixed-integer receding-horizon path planning.

This repository builds a compact research pipeline around multi-vehicle path planning MILPs: generate receding-horizon instances, collect expert branching decisions with SCIP/Ecole, train a bipartite GCNN policy, and evaluate it against standard SCIP branching baselines.
...
- PyTorch Geometric GCNN policy over the MILP bipartite graph.
- Four-way evaluation against VSB, FSB, SCIP DEFAULT, and the trained GCNN brancher.
...
| `03_train_gnn.py` | Train the GCNN branching policy. |
...
The implementation follows the standard learned-branching workflow introduced for SCIP/Ecole-style GCNN branchers and adapts it to receding-horizon multi-vehicle path-planning MILPs.
